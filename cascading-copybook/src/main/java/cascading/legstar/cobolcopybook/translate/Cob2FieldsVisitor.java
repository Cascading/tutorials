...
public class Cob2FieldsVisitor
   ...
   */
  protected void processCollectionElements( final XmlSchemaObjectCollection items, final int level, final String prefix, final String suffix ) throws Cob2FieldsException
    {
    ...
    for( int i = 0; i < items.getCount(); i++ )
      ...
      if( element instanceof XmlSchemaElement )
        {
        processElement( (XmlSchemaElement) element, level, prefix, suffix );
        }
        ...
        XmlSchemaGroup group = (XmlSchemaGroup) schema.getGroups().getItem( groupRef.getRefName() );
        processParticle( group.getName(), group.getParticle(), level, prefix, suffix );
        }
        ...
        for( int j = 0; j < alternatives.getCount(); j++ )
          ...
          if( alternative instanceof XmlSchemaElement )
            {
            String alternativeName = getJaxbName( ( (XmlSchemaElement) alternative ).getName() );
            if( firstAlternativeName == null )
              {
              firstAlternativeName = alternativeName;
              }
            String newPrefix = prefix + String.format( CHOICE_PREFIX_FORMAT, firstAlternativeName, alternativeName );
            processElement( (XmlSchemaElement) alternative, level, newPrefix, suffix );
            }
          }
        }
   ...
   * @throws Cob2FieldsException if processing fails
   */
  public void processElement( final XmlSchemaElement xsdElement, final int level, final String prefix, final String suffix ) throws Cob2FieldsException
    {
        /*
    ...
    if( xsdElement.getSchemaType() instanceof XmlSchemaComplexType )
      {
      XmlSchemaComplexType xsdComplexType = (XmlSchemaComplexType) xsdElement.getSchemaType();
      ...
      if( xsdElement.getMaxOccurs() > 1 )
        ...
        for( int i = 0; i < xsdElement.getMaxOccurs(); i++ )
          ...
          getProcessor().processElement( schema, xsdElement, level, prefix + getLabel( xsdElement, newSuffix ) );
          processComplexType( xsdComplexType, level, prefix, newSuffix );
          }
        }
      else
        {
        getProcessor().processElement( schema, xsdElement, level, prefix + getLabel( xsdElement, suffix ) );
   ...
   * @throws Cob2FieldsException if processing fails
   */
  public void processComplexType( final XmlSchemaComplexType xsdComplexType, final int level, final String prefix, final String suffix ) throws Cob2FieldsException
    {

    log.debug( "process started for complex type = " + xsdComplexType.getName() );

        /* Delegate processing of XML schema complex type */
    getProcessor().processComplexType( schema, xsdComplexType, level, prefix, suffix );

        /* Complex types might have particles such as sequence and all */
    XmlSchemaParticle particle = xsdComplexType.getParticle();
    processParticle( xsdComplexType.getQName(), particle, level + LEVEL_INCREMENT, prefix, suffix );

    log.debug( "process ended for complex type = " + xsdComplexType.getName() );
    }
   ...
   * @throws Cob2FieldsException if processing fails
   */
  public void processParticle( final QName parentName, final XmlSchemaParticle particle, final int level, final String prefix, final String suffix ) throws Cob2FieldsException
    {

    if( particle == null )
      {
      log.warn( "Schema object " + parentName + " does not contain a particle" );
      return;
      }
    ...
    if( particle.getMaxOccurs() > 1 )
      ...
      log.warn( "Schema object " + parentName + " contains a multi-occurence particle that is ignored" );
      }

    if( particle instanceof XmlSchemaSequence )
      {
      XmlSchemaSequence sequence = (XmlSchemaSequence) particle;
      processCollectionElements( sequence.getItems(), level, prefix, suffix );
      ...
      log.warn( "Schema object " + parentName + " does not contain a sequence or all element" );
      }
   ...
   */
  public String getLabel( final XmlSchemaElement xsdElement, final String suffix )
    {
