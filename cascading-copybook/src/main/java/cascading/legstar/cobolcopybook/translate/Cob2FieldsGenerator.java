...
public class Cob2FieldsGenerator {
    ...
    private static Logger log = LoggerFactory
            .getLogger(Cob2FieldsGenerator.class);

    private final Template hbtFieldsJava;

    public Cob2FieldsGenerator() {
        try {
            URL url = Resources.getResource( getClass(),
              FIELDS_JAVA_TEMPLATE_NAME );
    ...
    public void generate(File xsdInput, File outputDir, String packageName) {
        ...
        try {
            ...
            for (Entry < String, Fields > entry : mapFields.entrySet()) {
                ...
                // Build a model
                String className = CaseFormat.LOWER_UNDERSCORE.to(
                        CaseFormat.UPPER_CAMEL, entry.getKey());
                ...
                FileUtils.writeStringToFile(
                        new File(outputDir, packageName.replace(".",
                                File.separator)
                                + File.separator
                                + className
                                + ".java"), content);
